...
public class FittingCharacterStats : CharacterStats
{
    ...
    public Dictionary<FTK_itembase.ID, CharacterOverworld> OwnershipMap { get; set; }
    ...
    {
        if (!CachedDB.TryGetModByItemId(_item, out var mod, out var modId))
        {
            return;
        }
        ...
        if (_add)
        {
            if (!this.m_CharacterMods.Contains(modId))
            {
                this.m_CharacterMods.Add(modId);
                // reflect potential changes in party defense that is included in armor/resist calculations
                this.PartyArmorMod += mod.m_PartyCombatArmor;
                this.PartyResistMod += mod.m_PartyCombatResist;
                this.PartyEvadeMod += mod.m_PartyCombatEvade;
            }
        }
        else if (FTK_characterModifierDB.GetDB().IsContainID(_item.ToString()))
        {
            if (this.m_CharacterMods.Contains(modId))
            {
                this.m_CharacterMods.Remove(modId);
                this.PartyArmorMod -= mod.m_PartyCombatArmor;
                this.PartyResistMod -= mod.m_PartyCombatResist;
                this.PartyEvadeMod -= mod.m_PartyCombatEvade;
            }
