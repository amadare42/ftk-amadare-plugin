...
public class MaximizeStatService
{
    ...
    public Dictionary<StatType, FittingCharacterStats> GetMaximizeStatLoadout(CharacterOverworld cow, List<CharacterOverworld> linkedCows)
    {
        var r = this.accessorsMap.ToDictionary(pair => pair.Key,
            pair =>
            {
                var distributedLoadout =  InventoryCalculatorHelper.CalculateBestForStat(cow, linkedCows, pair.Key);
